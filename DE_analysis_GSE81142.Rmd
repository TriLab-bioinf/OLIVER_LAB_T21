...
# Separate M and F
```{r}
dds.f <- dds.gse81142[, dds.gse81142$Sex == "F" & dds.gse81142$Sample_Acc != "SRR3477024"] # I removed SRR3477024 because it clusters separated from the rest of the Female samples and very close to Male samples and therefore, it seems a misclassification.
...
# PCA
pcaData.f <- DESeq2::plotPCA(vsd.f, intgroup=c("DMSO","Time"), returnData=TRUE, ntop = 500)
percentVar <- round(100 * attr(pcaData.f, "percentVar"))
y.coords = c(min(pcaData.f$PC1, pcaData.f$PC2), max(pcaData.f$PC1, pcaData.f$PC2))
x.coords = y.coords
p1 <- ggplot(pcaData.f, aes(PC1, PC2, shape=DMSO, color=Time)) +
...
get_diff_exp_genes_across_conditions_plot <- function(DE_result_list){
  ...
  my_tibble_long <- pivot_longer(my_tibble, cols = 3:14, names_to = "contrast", values_to = "log2fc")
...
ggsave2(filename = "perturbed_genes_female.pdf", plot = p.f, path = "./Plots", width = 21, height = 8)
p.f

p.m <- get_diff_exp_genes_across_conditions_plot(DE_result_list = DE_results.m)
ggsave2(filename = "perturbed_genes_male.pdf", plot = p.m, path = "./Plots", width = 21, height = 8)
p.m
```

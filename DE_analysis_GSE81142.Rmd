...
design(dds.f) <- ~ Time + DMSO + Time:DMSO
dds.f <- DESeq(dds.f)
dds.f$dds_ID = "female"
resultsNames(dds.f)

# [1] "Intercept"     "Time_2_vs_0"   "Time_4_vs_0"   "Time_8_vs_0"   "DMSO_0.1_vs_0" "DMSO_0.2_vs_0"
# [7] "DMSO_0.4_vs_0" "Time2.DMSO0.1" "Time4.DMSO0.1" "Time8.DMSO0.1" "Time2.DMSO0.2" "Time4.DMSO0.2"
#c[13] "Time8.DMSO0.2" "Time2.DMSO0.4" "Time4.DMSO0.4" "Time8.DMSO0.4"
...
design(dds.m) <- ~ Time + DMSO + Time:DMSO
...
comparisons <- list(
  ...
  c("DMSO_0.2_vs_0"),
  c("DMSO_0.4_vs_0"),
  c("DMSO_0.1_vs_0","Time2.DMSO0.1"), c("DMSO_0.1_vs_0","Time4.DMSO0.1"), c("DMSO_0.1_vs_0","Time8.DMSO0.1"),
  c("DMSO_0.2_vs_0","Time2.DMSO0.2"), c("DMSO_0.2_vs_0","Time4.DMSO0.2"), c("DMSO_0.2_vs_0","Time8.DMSO0.2"),
  c("DMSO_0.4_vs_0","Time2.DMSO0.4"), c("DMSO_0.4_vs_0","Time4.DMSO0.4"), c("DMSO_0.4_vs_0","Time8.DMSO0.4")
)
########################################################################################################
...
```{r}
dds.m.t4 <- dds.m[,dds.m$Time == "4"] # I removed SRR3477078 because had very low number of reads and clustered separated from the rest of the Male samples.
dds.m.t4$Time <- droplevels(dds.m.t4$Time)
design(dds.m.t4) <- ~ DMSO
dds.m.t4$DMSO <- relevel(dds.m.t4$DMSO, "0")
dds.m.t4 <- DESeq(dds.m.t4)
dds.m.t4$dds_ID = "male_t4"
resultsNames(dds.m.t4)

i=c("DMSO_0.2_vs_0")
res.m.t4 <- get_deseq_result(dds = dds.m.t4, contrast = i, analysis_type = "ashr" )
summary(res.m.t4, alpha = 0.05)
#res.m.t4 <- results(dds.m.t4, list( i ))
ix = which.min(res.m.t4$padj) # most significant
res.m.t4 <- res.m.t4[order(res.m.t4$padj),] # sort
barplot(assay(dds.m.t4)[ix,],las=2, main=rownames(dds.m.t4)[ ix  ]  )

genes_of_interest <- head(rownames(res.m.t4))
plot_normalized_gene_expression(my_dds = dds.m.t4, 
                                ensmbl_gene_list = genes_of_interest, 
                                file_prefix = "Genes_with_extreme_up_down_expr")
```
